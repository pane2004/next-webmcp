...
function usePendingConfirmation(): PendingConfirmation | undefined {
 ...
 * Mount once, near the root layout. Enter approves and Escape denies while focus is on the card
 * (the Approve button is focused automatically) or nowhere; keys typed into other inputs are ignored.
 ...
 * ```tsx
 * // app/layout.tsx
 * <body>{children}<ToolConfirmations /></body>
 * ```
...
export function ToolConfirmations(): React.JSX.Element | null {
  ...
  const queued = useSyncExternalStore(
    registry.subscribe,
    () => registry.getState().confirmations.length,
    () => 0,
  );
